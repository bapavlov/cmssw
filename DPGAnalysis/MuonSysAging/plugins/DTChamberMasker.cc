...
private:
  ...
  void createMaskedChamberCollection(const DTGeometry& dtGeom);
  ...
  edm::ESGetToken<MuonSystemAging, MuonSystemAgingRcd> agingToken_;  
  ...
  m_doTriggerFromDDU( iConfig.getParameter<bool>("doTriggerFromDDU") ) 
{
  ...
  agingToken_ = esConsumes<MuonSystemAging, MuonSystemAgingRcd>();
...
DTChamberMasker::beginRun(edm::Run const& run, edm::EventSetup const& iSetup)
{
  ...
  if (m_enabledCh.size() == 0)
    {
      const MuonSystemAging& mcData = iSetup.getData(agingToken_);
      std::vector<std::string> mcV = mcData.m_DTchambers;
...
void DTChamberMasker::createMaskedChamberCollection(const DTGeometry& dtGeom)
{
